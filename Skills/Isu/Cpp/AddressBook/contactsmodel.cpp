...
{
    const bool updateResult {updateConstacts()};
    if(!updateResult){
        qWarning() << "Update constacts failed!";
    }
}
...
QVariant ContactsModel::data(const QModelIndex &index, int role) const
{
    ...
    const Contact& contacts {m_contacts.at(index.row())};
    switch (role) {
    case ContactRoles::NameRole: {
        return QVariant::fromValue(contacts.firstName());
    }
    case ContactRoles::SurnameRole: {
        return QVariant::fromValue(contacts.surname());
    }
    case ContactRoles::PhoneNumberRole: {
        return QVariant::fromValue(contacts.phone());
    }
    default: {
        return true;
    }
...
bool ContactsModel::updateConstacts()
{
